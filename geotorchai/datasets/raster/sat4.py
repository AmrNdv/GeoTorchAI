...
class SAT4(Dataset):
    ...
    ADDITIONAL_FEATURES = ["contrast", "dissimilarity", "homogeneity", "energy", "correlation", "ASM", "mean_NDWI",
                           "mean_NDVI", "mean_RVI"]
    ...
    def __init__(self, root, is_train_data=True, bands=SPECTRAL_BANDS, include_additional_features=False,
                 additional_features_list=ADDITIONAL_FEATURES,
                 user_features_callback: Optional[Dict[str, Callable]] = None, transform: Optional[Callable] = None,
                 target_transform: Optional[Callable] = None, mode = None):
        super().__init__()
        ...
        self._feature_callbacks = [ttf._get_GLCM_Contrast, ttf._get_GLCM_Dissimilarity, ttf._get_GLCM_Homogeneity,
                                   ttf._get_GLCM_Energy, ttf._get_GLCM_Correlation, ttf._get_GLCM_ASM]
        ...
        if is_train_data:
            ...
            self.x_data = torch.tensor(df.values.reshape((len(df), 28, 28, 4)))  # Changed to len(df)
            ...
            if mode is None:
              ...
              self.y_data = torch.tensor(df.values)
            elif mode =="construction":
                self.y_data = torch.tensor(df.values.reshape((len(df), 28, 28, 1)))  # Changed to len(df)
                self.y_data = torch.moveaxis(self.y_data, -1, 1)
        else:
                    ...
                    if self.user_features_callback != None and ad_feature in self.user_features_callback:
                        ...
                        if len(feature_index) > 0:
                            ...
                            raise InvalidParametersException(
                                "Callback not found for some user-defined feature: " + ad_feature)
